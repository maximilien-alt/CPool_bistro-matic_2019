...
char *is_this_negative(char *str, char *div, char *result)
{
    if (*div == '-' && *div != *str) {
        result = my_print_neg1(str);
        return (result);
    }
    return (str);
}
...
char    *my_infin_mod(char *str, char *div)
{
    ...
    if (my_infin_cmp(my_delete_neg(str), my_delete_neg(div)) == 1) {
        ...
        result = my_strcat(result, tempo);
        result = my_str_delete_null(result);
        if ((*str == '-' || *div == '-') && *str != *div)
            result = my_print_neg1(result);
        return (result);
    } else
        return (is_this_negative(str, div, tempo));
}